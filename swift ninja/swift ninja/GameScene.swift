...
class GameScene: SKScene {
    ...
    override func didMove(to view: SKView) {
        ...
        pauseButton.position = CGPoint(x: 77, y: 710)
        pauseButton.blendMode = .replace
    ...
    func createEnemy(forcebomb: ForceBomb = .random) {
        ...
        enemy.physicsBody = SKPhysicsBody(circleOfRadius: 64)//bc 128 for bomb
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if let touch = touches.first {
            ...
            for node in tappedNodes {
                ...
                if node.name == "pause"{
                    guard let gamepause = scene?.view?.isPaused else {return}
                    scene?.view?.isPaused = !gamepause
                }
            }
    ...
    override func touchesMoved(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        for node in nodesAtPoint {
            if node.name == "enemy" {
                ...
                emitter.position = node.parent!.position //bomb inside bomb image which is inside bomb container
                addChild(emitter)
